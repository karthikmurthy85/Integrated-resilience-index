dLogistic <- function(ndvi){
  ...
  for(i in 1:length(lowc)){
    ...
    optimal <- optim(c(vb,ve,k,p,d,c,q),fn=Wx,gr=NULL,method="L-BFGS-B",
                     lower=c(0, (0), 0, 2, lowc[i], lowd[i], (pk+1)),
                     upper=c((vb + 0.001), (ve + 0.002), (max(ndvi)+ 0.1), (pk-5), 0.2, 0.2, 
                             (length(ndvi)-2)),
  ...
  optimal <- optim(c(vb,ve,k,p,d,c,q),fn=Wx,gr=NULL,method="L-BFGS-B",
                   lower=c(0, (0), 0, 2, fitdat1$X1[1], fitdat1$X1[1], (pk+1)),
                   upper=c((vb + 0.001), (ve + 0.002), (max(ndvi)+ 0.1), (pk-5), 0.2, 0.2, 
                           (length(ndvi)-2)),
                   control=list(maxit = 5000, pgtol = 1e-10, 
                                ndeps = c(1e-10, 1e-10, 1e-10, 1, 1e-10, 1e-10,1), 
